...
class EventedProperty {
  ...
  setObject(objectToSet) {
    ...
    this.on((value, { fullPathNames }) => {
      ...
      var _fullPathNames = _toArray(fullPathNames);

      let omit = _fullPathNames[0],
          remainingFullPathNames = _fullPathNames.slice(1);
...
function setObjectBasedOnFullPath({ objectToSet, fullPathNames, value }) {
  ...
  if (fullPathNames.length == 1) {
    ...
    var _fullPathNames2 = _toArray(fullPathNames);

    let discard = _fullPathNames2[0],
