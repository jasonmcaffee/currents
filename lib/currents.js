...
exports.setObjectBasedOnFullPath = setObjectBasedOnFullPath;
...
class EventedProperty {
  ...
  setObject(objectToSet, { setValue = defaultSetValue } = {}) {
    ...
    this.on((value, eventedProperty) => {
      let fullPathNames = eventedProperty.fullPathNames,
          setValue = eventedProperty.setValue;
      //omit the first fullPathName, since that represents the objectToSet.
      //e.g. eventBus.person({objectToSet: person});

      var _fullPathNames = fullPathNames,
          _fullPathNames2 = _toArray(_fullPathNames);

      let omit = _fullPathNames2[0],
          remainingFullPathNames = _fullPathNames2.slice(1);
  ...
  handleAction(action) {
    ...
    let on = action.on,
        ...
        onSetValue = action.onSetValue,
        rest = _objectWithoutProperties(action, ['on', 'off', 'fire', 'setObject', 'onSetValue']);
 ...
 */
function setObjectBasedOnFullPath({ objectToSet, fullPathNames, value, parentObject, objectToSetPropertyNameOnParent, setValue = defaultSetValue }) {
  ...
  if (fullPathNames.length == 1) {
    ...
    var _fullPathNames3 = _toArray(fullPathNames);

    let objectToSetPropertyNameOnParent = _fullPathNames3[0],
        remainingFullPathNames = _fullPathNames3.slice(1);
